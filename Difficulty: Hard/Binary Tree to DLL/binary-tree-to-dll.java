...
class Solution {
    // Node to keep track of the previous node in DLL.
    Node prev = null;
    ...
    Node bToDLL(Node root) {
        ...
        // Initialize the head of DLL
        Node head = null;
        
        // Convert the left subtree and find the head of the DLL
        head = bToDLL(root.left);
        
        // If prev is null, then this is the leftmost node (head of the DLL)
        if (prev == null) {
            head = root;
        } else {
            // Connect the previous node with the current root node
            root.left = prev;
            prev.right = root;
        }
